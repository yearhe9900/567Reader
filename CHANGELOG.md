...
* 书籍信息页模糊封面背景降低放大比例：`BookCover.loadBlur` 新增可选 `fitCenter` 参数（默认 `false`，沿用原 `CenterCrop` 全幅裁切，音阅播放页不受影响）；书籍信息页传 `fitCenter = true` 改为 `CenterInside`——竖版封面不再被拉满宽幅带裁掉约 55% 高度，改为完整显示封面（轻微缩放、无裁切），配合原有竖向渐变遮罩。仅书籍信息页生效，其余调用方行为不变。
