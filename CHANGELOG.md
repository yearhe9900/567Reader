...
* 书籍信息页模糊封面背景改为与 legado 一致的全屏方案：参考 `activity_book_info.xml` 布局——`bg_book` 为 `match_parent × match_parent`（全屏）`centerCrop`，`vw_bg` 为 `#50000000`（31%黑色）全屏半透明遮罩。Compose 移植：模糊背景 `AndroidView` 改为 `fillMaxSize()` + `CenterCrop`（回退此前的 `fitCenter`/`CenterInside`），渐变遮罩改为全屏 `Color(0x50000000)` 平面遮罩。内容区 `background(bgColor)` 自然遮住下方模糊，效果与 legado 一致。`BookCover.loadBlur` 的 `fitCenter` 参数保留但不再使用。
* 搜索页回归 legado 忠实行为：回退 09/07 的「惰性预抓详情补全分类」（`SearchViewModel.fillKindOne` 整段删除，含 `fillingKinds`/`kindSemaphore`/`Semaphore(4)` 并发与 `bookUrl` 去重；`SearchBookItem` 的 `onFillKind` + `LaunchedEffect` 入口一并移除）。理由：legado 搜索列表本身不补分类，预抓是偏离行为；搜索结果项回到 legado 原貌——`kind` 为空时只剩「123字」、有 `kind` 时显示「123字 玄幻 都市…」。
* 搜索列表隐藏孤零零的字数标签：`SearchBookItem` 的 `kinds = searchBook.getKindList()` 改为——`kind` 为空（即只有 `wordCount`）时置空列表，配合 `if (kinds.isNotEmpty())` 直接不渲染 `LabelsBar`，避免只挂一个「123字」。这轻微偏离 legado（legado 会显示「123字」），是用户明确要求的小幅美化；有真实分类时仍照 legado 显示「123字 + 分类」。
* 修复搜索页「单书源搜索停止/开始死循环」Bug：选单书源搜索时，结果往往一屏全可见。`snapshotFlow { listState.layoutInfo }` 在数据首次到达时即发射，`lastVisible.index >= totalItemsCount - 3` 命中、`hasMore=true` → 自动 `search("")` → `searchPage++` → 第 2 页结果同样一屏全可见 → 再次自动加载……形成「停止-开始」死循环。根因：Compose 移植时漏了 legado `RecyclerView.addOnScrollListener` 的「只在用户实际滚动时回调（`onScrolled`）」语义，`snapshotFlow` 对所有布局变化都发射。修复：自动加载更多条件加上 `listState.isScrollInProgress` 闸门——只在用户实际滚动（拖拽/ fling）时才触发 `scrollToBottom`，数据首次到达 / 加载更多插入新条目均不触发，与 legado `OnScrollListener` 语义一致。`SearchModel.kt` 与 legado-E 逐行一致（`hasMore = items.isNotEmpty()` 同源），未改。
* 修复书籍信息页标题栏丢失「编辑」与「更多选项」按钮：Compose 化时标题栏只移植了返回箭头与标题，Activity 虽仍 inflate `R.menu.book_info`，但原生 options menu 已无处显示，导致编辑/刷新/登录/置顶/变量/复制链接/允许更新/清缓存/删除提示等入口全部不可达。修复：①标题栏右侧补「编辑」（`ic_edit`，仅书籍在书架时可见，与 legado `menu_edit` 一致）与「更多选项」（`ic_more_vert`）；②Activity 把 `onMenuOpened` 的可见性/勾选计算抽成 `upMenuState(menu)`，新增 `composeMenu`（`MenuBuilder` 载入同一份 `R.menu.book_info`）在展开时构建 `BookInfoMenuAction` 列表交给 Compose 渲染，保证与原生菜单同源同规则；③`onCompatOptionsItemSelected` 的分支逻辑抽成 `handleMenuAction(id, checked)`，原生 MenuItem 与 Compose 菜单共用，勾选态由调用方传入（不再依赖 `MenuItem` 对象）；④`GlassDropdownMenu` / `GlassDropdownMenuItem` 从 MainScreen 提取为 `ui/compose/glass/` 共享组件（新增可选图标与勾选标记支持），书籍信息页与书架页共用同一套玻璃下拉。
* 修复发现页文字乱码：根因是 `MainScreen.kt` 曾被以错误编码保存，部分中文字节被破坏成 U+FFFD（全项目仅此一个文件受损）。受影响的 UI 文案是发现页搜索框 placeholder `搜索书源…` 与分组菜单空态 `无分组`（另有 76 处注释同样受损）。修复方式：从尚未损坏的提交 `5305fd6` 取出干净版本重建文件，再把损坏之后新增的唯一一处实质代码 `GlassConfig.SyncStatusBarToGlassTheme(isLightTheme = isLightTheme)` 重新应用回去（该提交相对干净版只有这一行代码改动，其余全是编码破坏）。
* 发现页分组菜单玻璃态化：原先是 material3 `DropdownMenu`，挂在标题栏内 40dp 的小 Box 上，既非玻璃态又会挤压弹出范围。改为与书架页共用 `GlassDropdownMenu` / `GlassDropdownMenuItem`：菜单开关状态 `exploreGroupMenuOpen` 与分组数据 `flowExploreGroups()` 提升到 `MainScreen`，菜单本体渲染在**全屏外层 Box**（backdrop 捕获层之外），标题栏只保留触发回调 `onGroups`；空态文案改用 `R.string.no_group` 走国际化。
* 分组弹框（主界面「分组」玻璃弹框）图标由文件夹图标改为**组内前 4 本封面组成的 2×2 四格拼图**：每个分组通过 `appDb.bookDao.flowByGroup(group.groupId)` 取组内图书，按 `durChapterTime` 降序（最近阅读优先）取前 4 本，四格各自用 `AndroidView(ImageView) + BookCover.load()`（Glide，与搜索页封面同一套）加载；组内不足 4 本时缺位由 `R.drawable.image_cover_default` 默认封面补齐（Glide 的 placeholder/error 同为默认封面，空 URL 亦自动回落）。选中态改为四格容器叠 `accentColor@0.25f` 高亮，宫格最大高度 350dp → 420dp 以容纳变高的分组项。特殊分组（全部/本地/有声/未分组 等）走 `flowByGroup` 的既有分支，同样生效。
* 玻璃配色收敛为单一真源：`GlassConfig` 新增 `isLightTheme(context)`（**按背景色 `backgroundColor` 判定而非主色** —— legado 主色常是用户选的深色强调色，按主色判定会把「浅背景+深主色」误判为暗色，导致玻璃永远深灰、文字永远白）与 `contentColor(isLightTheme)`；`GlassDialogTokens` 的容器色/内容色改为引用 `GlassConfig` 常量不再二次定义；MainScreen / ExplorePage / SearchScreen / BookInfoScreen / GroupEditOverlay 各自的 `ColorUtils.isColorLight` 判定与黑白字硬编码统一改为调用 `GlassConfig`。
* 修复书籍信息页「内容少时底部露出模糊封面」Bug（两次修复 d39c189 仍无效后的最终方案）：根因是 Compose 布局语义——在 `verticalScroll` Column 内部，`fillMaxSize()` 收到的 maxHeight 约束是 **Infinity**（滚动容器给内容无限高度），实际只等于内容自身高度，永远不会撑满视口；而 View 体系 `match_parent` 在 ScrollView 里会被解析为视口高度（legado 的 `ll_info` 因此天然触底）。Compose 没有 `fillViewport` 等价物（`weight(1f)` 在滚动容器内同样退化为按内容测高）。最终方案：`BoxWithConstraints` 取视口高度 + 封面/信息区与简介区分别 `onSizeChanged` 实测高度，滚动内容末尾放一个 `height((viewportHeight - contentHeight - introHeight).coerceAtLeast(0.dp))` 的 `bgColor` 填充 Box，内容不足一屏时补满剩余空间遮住模糊背景，内容超一屏时填充为 0 不影响滚动。WebView 简介异步加载后高度变化会再次触发 `onSizeChanged`，填充块自动收缩，无需手动干预。
